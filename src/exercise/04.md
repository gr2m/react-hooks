...
- **Managed State** vs **Derived State**: Managed state is explicit, using
  `React.useState()`. Derived state is calculated based on managed state.
- When setting state, the state will only be set after the render. In order to
  work with the new value of a state, it needs to be stored in a separate
  variable, then use that variable in the event handler and for setting new
  state.
- A derived state is the return value of a function that accepts a state. If the
  derived state is used in a render, the derive function is called automatically
  to calculate the new derived state.
