...
- when working with UI libraries that interact with the DOM, we need to create a
  reference using `React.useRef()` and set the `ref={ref}` prop on the element.
- `React.useEffect()` can return a function that is called when the dom element
  is unmounted.
- Use a reference whenver you want to make changes after the render. If the
  change should be part of a render than a state should be used instead.
