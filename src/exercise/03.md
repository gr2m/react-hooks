...
- **Lifting state** means to keep a single source of truth with the closest
  common parent component. Components that lift state accept props for
  `state`/`setState` as returned by `React.useState()`.
- **Colocating state** means the oposite of lifting state: move state down to
  the components that need it. The benefit is more efficient rendering, as React
  won't re-render the entire tree if a state changes somewhere down it's node
  tree. If the state is kept on the very top (e.g. using Redux), then the whole
  tree needs to be re-rendered, which often times is not necessary.
