...
- The callback passed to `React.useEffect(callback)` is called after each render
  and re-render of the component.
- `React.useEffect(callback)` accepts a 2nd argument: an array "dependencies".
  One such "dependency" can be a state variable returned from
  `React.useState()`.
- Lazy state initialization: `React.useState(callback)`. The callback is called
  only once, when the component is first rendered.
- When moving generic behavior into a module that uses React hooks, the
  convention is that that module returns a function with the name having a `use`
  prefix.
- The extra credit 4 (accept all types) was a bit overkill. I get what it was
  about but I didn't feel that it would benefit me much so I skipped it. The
  code in `src/final/02.extra-4.js` is quite complex, it could use some comments
  / examples.
